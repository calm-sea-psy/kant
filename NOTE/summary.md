...
주제별 키워드 요약입니다. 매일 위에서 아래로 훑어보며 리마인드하고, 더 깊은 설명이 필요하면 각 항목의 `상세 →` 링크로 해당 도메인 파일의 절로 바로 이동합니다.

- [수학](1-수학.md) · [머신러닝](2-머신러닝.md) · [딥러닝](3-딥러닝.md) · [LLM](4-LLM.md)

## 수학

### 선형대수와 행렬분해 · [상세 →](1-수학.md#선형대수와-행렬분해)
...
### 지도학습·비지도학습 기초 모델 · [상세 →](2-머신러닝.md#지도학습비지도학습-기초-모델)
...
- **실전 작업 흐름**: 단순 기준선 → 비선형 모델 개선 → 교차검증으로 편향/분산 점검 → 앙상블/튜닝

### 앙상블 학습 · [상세 →](2-머신러닝.md#앙상블-학습-부트스트랩부터-shap까지)
...
### 평가지표와 데이터 누수 · [상세 →](2-머신러닝.md#평가지표와-데이터-누수)
...
- **주석자 일치도**: Cohen's kappa 등, 낮으면 가이드라인이 모호하다는 신호. 골드셋(전문가 합의 정답)으로 평가·검증
...
### RNN과 LSTM · [상세 →](3-딥러닝.md#rnn과-lstm-순환-신경망과-게이트-구조)
...
### 트랜스포머 수학 · [상세 →](3-딥러닝.md#트랜스포머-수학-attention-shape미분역전파)
...
- **어텐션의 등장 배경**: RNN의 장거리 의존성 희석·고정 크기 병목·문맥 의존 의미·병렬 처리 불가를 해결. 먼 위치도 한 번에 직접 연결
- **Query·Key·Value 비유**: 검색어·색인·본문. Q와 K를 분리해 비대칭 관계 학습, K와 V를 분리해 매칭용 표현과 전달 내용을 구분
- **어텐션에서 학습되는 것**: 내적·softmax·가중합엔 파라미터 없음. Q/K/V projection과 출력 projection만 학습됨
- **계산 4단계**: 비교(내적) → 조절(차원 제곱근으로 나눔) → 비율(softmax, 경쟁적 배분) → 섞기(Value 가중합)
- **표현 축의 변화**: 입력은 토큰별 특징 벡터 → 비교 후 특징 차원이 사라지고 토큰×토큰 관계표만 → 섞기 후 문맥 반영된 특징 벡터로 복원
- **score·weight·context vector**: softmax 전은 score(logit), 후는 weight, Value 가중합 결과는 context vector. 이름은 Bahdanau seq2seq에서 유래
- **멀티헤드 어텐션**: 모델 차원을 나눠 여러 head가 서로 다른 관계를 병렬 포착. concat 후 출력 projection으로 섞음. head를 나눠도 총 연산량은 불변

## LLM

### 자연어 처리 기초 · [상세 →](4-LLM.md#자연어-처리-기초-텍스트를-모델-입력으로-바꾸기)

> 출처 TIL: 260902

- **NLP**: 사람 언어를 컴퓨터가 이해·분석·생성하게 만드는 분야. 지금은 Transformer 기반 딥러닝 중심
- **대표 과제**: 분류, 시퀀스 레이블링(NER·POS), seq2seq(번역·요약), 언어 모델링(다음 토큰 예측), 문장 임베딩·검색, 생성
- **역사 흐름**: 규칙 → 통계(n-gram·HMM·CRF) → 단어 임베딩 → RNN/LSTM+attention → Transformer → 사전학습 LM → LLM
- **텍스트→숫자 파이프라인**: 원문 → 정규화 → 토큰화 → 정수 인코딩(vocab) → 수치 표현(희소/밀집) → 배치화(패딩+마스크)
- **텍스트 계층**: 코퍼스 ⊃ 문서 ⊃ 문장 ⊃ 토큰. type(고유 단어 종류) vs token(등장 포함), 어휘 크기는 type 기준
- **토큰화 단위**: 단어(어휘 폭발·OOV), 문자(시퀀스 폭증), 바이트, 서브워드(절충안). 토큰 수 = 시퀀스 길이 = 연산량·컨텍스트·과금 단위
- **단어 토큰화의 한계**: run/runs/running 별도 취급, 한국어는 교착어라 어절·조사·어미 조합으로 폭발, OOV는 [UNK]로 소실
- **서브워드**: 자주 쓰는 건 통째로, 드문 건 조각으로. 어휘 크기 고정, OOV 사실상 제거, 형태소 규칙 학습
- **BPE**: 가장 자주 인접하는 쌍을 반복 병합, 병합 규칙 목록이 곧 어휘. GPT·Llama 계열
- **바이트 수준 BPE**: 바이트에서 시작해 모든 문자 커버, 진짜 OOV 없음 (GPT-2 이후)
- **WordPiece**: 빈도 대신 likelihood 증가량 기준 병합, 이어지는 조각에 ## (BERT)
- **Unigram LM**: 큰 어휘에서 시작해 덜 유용한 토큰 제거 (T5·다국어)
- **SentencePiece**: 공백 포함 raw text에서 바로 학습, 언어 독립적, 공백은 ▁, 완전 복원 가능 (Llama·Gemma·한일 모델)
- **Vocabulary**: 모델이 아는 토큰의 고정 집합. 토크나이저 학습 시 생성. 토큰↔ID 두 딕셔너리로 관리
- **어휘 크기 상충 관계**: 작으면 시퀀스↑·임베딩 행렬↓, 크면 반대. BERT 3만 / GPT-2 5만 / Llama2 3.2만 / Llama3 12.8만 / GPT-4o 20만
- **특수 토큰**: [PAD] [UNK] [CLS] [SEP] [BOS] [EOS] [MASK], 채팅 템플릿용 턴 구분 토큰
- **학습·추론 토크나이저 일치**: 어휘가 다르면 같은 ID가 다른 토큰을 가리켜 모델 붕괴 → 체크포인트와 짝으로 배포
- **ID**: 어휘 속 정수 번호(0~V-1). 신경망은 문자열 못 받음 + 임베딩 행렬의 행 인덱스로 쓰임. 번호 자체엔 의미 없음(의미는 임베딩이 담당)
- **정수 시퀀스·패딩**: 문장마다 길이가 달라 배치의 최장 길이에 맞춰 PAD(0번) 채움, attention_mask(1=실제, 0=패딩)로 무시 지시
- **추가 입력 필드**: token_type_ids(문장 A/B), position_ids(위치), labels(정답 — 분류/토큰별/한 칸 민 것)
- **희소 표현**: 원-핫, BoW(순서 소실), TF-IDF(희귀도 가중), n-gram(순서 부분 보존). 벡터 대부분 0 → 희소 행렬
- **밀집 표현(임베딩)**: 단어를 저차원 실수 벡터로, 의미 가까우면 벡터도 가까움. 문맥 임베딩(BERT)은 같은 단어도 문맥별 다른 벡터. 문장 임베딩은 검색·RAG용
- **벡터 인덱스·벡터 DB**: (ID, 벡터, 메타데이터) 레코드 + 근사 최근접 이웃 검색(FAISS·HNSW). RAG의 핵심 자료구조
- **구조적 표현**: 구문 분석 트리, 의존 구문 트리, 지식 그래프 트리플. LLM 이후 모델 내부 학습으로 이동
- **Transformer 내부 자료구조**: 어텐션 행렬(seq×seq), Q·K·V 표현, KV 캐시(과거 K·V 버퍼 → 추론 속도 핵심)
- **토크나이저**: 텍스트↔ID 왕복 변환 컴포넌트. 학습(코퍼스→어휘·병합규칙, 1회) vs 사용(encode/decode, 매번)
- **인코딩 파이프라인 5단계**: 정규화 → 사전 토큰화(경계 확정, 서브워드가 못 넘음) → 모델(BPE/WordPiece/Unigram) → 후처리(특수 토큰 부착) → ID 변환
- **디코딩**: ID → 토큰 → ▁·바이트 되돌려 문자열 조립. 보통 원문 복원되나 강한 정규화 시 불일치 가능
- **토큰화 실무 영향**: 과금·컨텍스트는 토큰 단위(문자 수 아님), 한국어는 1.5~2배, 토큰 경계가 글자 세기·숫자 연산 약점 유발, 희소 학습 토큰은 이상 동작

### 트랜스포머 아키텍처 · [상세 →](4-LLM.md#트랜스포머-아키텍처-전체-구조인코더디코더추론)

> 출처 TIL: 260904

- **Transformer 정의**: RNN·CNN 없이 어텐션+FFN만으로 시퀀스 처리. 동일 블록 stack + 잔차·정규화 + 위치 인코딩 + 입출력층. 원조는 인코더-디코더
- **Self-Attention vs Transformer**: 셀프 어텐션은 연산(서브층) 하나로 토큰끼리 정보만 섞음, Transformer는 그 위에 FFN·임베딩·head·정규화를 얹은 완결된 모델. 엔진 vs 자동차
- **섞기와 가공의 반복**: 어텐션은 토큰 내용을 거의 안 바꿈, 실제 가공은 FFN. "토큰끼리 섞기 → 토큰별 가공" 한 쌍을 여러 번 반복
- **인코더**: 원문을 양방향 셀프 어텐션으로 한 번에 읽어 문맥 벡터열(인코더 메모리) 생성, 모든 디코딩 스텝에서 재사용
- **디코더**: 마스크드 셀프 어텐션(미래 차단) + 교차 어텐션(Q=디코더, K·V=인코더) + FFN. 마지막에 선형층+softmax로 다음 토큰 확률
- **어텐션 3종류**: 인코더 셀프(양방향), 디코더 셀프(causal), 교차(디코더 Q + 인코더 K·V)
- **세 갈래**: 인코더 전용(BERT, 이해), 디코더 전용(GPT·Llama, 생성), 인코더-디코더(T5·BART, seq2seq)
- **네 구간**: 입력 준비(임베딩+위치) → 문맥화(셀프 어텐션, 토큰 간 정보 이동 유일) → 표현 가공(FFN, 토큰별 독립, 파라미터 약 2/3) → Task 출력(과제별 head, 파인튜닝 경계)
- **블록 = 문맥화 + 표현 가공**, 여러 번 반복. 앞층은 구문, 뒤층은 의미
- **위치 인코딩 필요성**: 셀프 어텐션은 순열 등변이라 순서를 모름 → 위치 벡터를 토큰 임베딩에 더함
- **Sinusoidal vs Learned**: sinusoidal은 고정 함수(짝/홀 차원 sin/cos, 차원마다 주기 다름), learned는 역전파로 학습. 둘 다 절대 위치, 모델 차원, 임베딩에 1회 더함
- **Encoder-only**: 인코더 스택만, 양방향, causal mask 없음. 이해·분류·검색·추출용, 자기회귀 생성 불가
- **Decoder-only**: 디코더에서 교차 어텐션 제거, 마스크드 셀프 어텐션 + FFN. 언어 모델링(입력을 한 칸 민 것이 정답)
- **Causal Mask**: 점수 행렬 상삼각(미래 위치)에 큰 음수를 더해 softmax 후 비중 0. 곱이 아닌 덧셈(softmax 이전), 무한대 대신 큰 음수(NaN 방지)
- **Causal Mask와 병렬 학습**: teacher forcing — 정답 시퀀스를 통째로 넣고 마스크로 시야만 제한 → 한 번의 순전파로 여러 위치 예측 동시 학습. Transformer 학습이 빠른 이유
- **Causal Mask가 만드는 성질**: 위치 i 표현은 0~i에만 의존 → 뒤 토큰 추가돼도 앞 표현 불변 → KV 캐시 성립, 단방향 흐름
- **FFN**: 선형→비선형(ReLU/GELU/SwiGLU)→선형, 차원 4배로 부풀렸다 줄임. 비선형 표현력·지식 저장 담당, 파라미터 약 2/3, 토큰별 독립
- **잔차 연결**: 서브층 출력에 입력을 그대로 더함. gradient가 감쇠 없이 아래층까지 도달, 항등 함수 학습 쉬움 → 수십~수백 층 학습 가능
...
### 사전학습 언어모델 · [상세 →](4-LLM.md#사전학습-언어모델-plmmlmclm)
...
### 모델 적응 방법 (프롬프트에서 파인튜닝까지) · [상세 →](4-LLM.md#모델-적응-방법-프롬프트에서-파인튜닝까지)

> 출처 TIL: 260909, 260911

- **적응 스펙트럼**: "가중치를 얼마나 건드리는가" 축 — 프롬프트 엔지니어링·prompt-only(0%) → 프롬프트 튜닝(soft prompt) → PEFT(1~5%) → 전체 미세조정(100%). 오른쪽일수록 비용↑, 행동 변경력↑
- **프롬프트 엔지니어링**: 가중치 불변, 입력 설계로 출력 유도(지시·few-shot·CoT·형식 지정). 비용 0·즉시. 한계는 토큰 비용·윈도우·지식량·본래 능력 이상 불가. +RAG로 지식 보완
- **prompt-only**: 경사하강 전혀 없이 frozen 모델+프롬프트. hard prompt는 사실상 프롬프트 엔지니어링, 논문 비교표의 baseline. 파라미터 업데이트 0
- **프롬프트 튜닝**: 모델 freeze + 입력 앞 학습 가능한 연속 벡터(soft prompt) 수십 개만 학습. 과제당 수 KB~MB. prefix-tuning·P-tuning v2 동류. 학습 파라미터 0.01~0.1%, 큰 모델일수록 full FT와 격차↓
- **PEFT**: 대부분 freeze + 소수만 학습하는 방법 총칭(LoRA·QLoRA·Adapter·prompt/prefix·(IA)³·BitFit). 메모리·체크포인트 절감, 망각 완화, 성능 근접. 오픈모델 커스터마이징 표준
- **전체 미세조정**: 모든 가중치 역전파 갱신. 최고 잠재 성능·근본적 행동 변경, 대신 파라미터당 ~16B 메모리, 과제마다 전체 사본, 과적합·망각 위험
- **시도 순서**: 프롬프트(+RAG) → LoRA/QLoRA(형식·스타일·반복 과제 한계+라벨 데이터) → 프롬프트 튜닝 계열(다중 과제 초경량 스위칭) → 전체 미세조정·계속 사전학습(대규모 데이터+근본 변경)
- **동결 vs 학습 파라미터**: requires_grad로 구분. 학습 파라미터 1개 ≈ 16B(가중치+경사+Adam 상태 2), 동결은 가중치만 → 학습 대상 1%로 줄이면 옵티마이저 메모리 ~100배↓. 단 활성값 메모리는 별개
- **PEFT 세 갈래**: 추가형(Adapter·LoRA·소프트 프롬프트), 선택형(BitFit·특정 층), 재매개변수화(LoRA). 표준 구현은 허깅페이스 peft
- **LoRA**: 가중치 갱신량을 작은 행렬 A·B의 곱으로. r(4~64)로 줄였다 되돌림, W는 freeze A·B만 학습, A=가우시안·B=0 초기화(시작 시 보정 0). q·v 투영에 부착. 추론 시 W에 병합하면 지연 0
- **QLoRA**: 베이스 모델 4비트 양자화해 freeze + 그 위 LoRA → 24GB GPU로 650억급 튜닝
- **Adapter**: 층 안(어텐션·FFN 뒤)에 병목 모듈(down→비선형→up→잔차) 삽입, 그것만 학습. 전체의 1~5%. LoRA에 밀린 이유는 추론 지연(병합 불가, 매 순전파 추가 비용)
- **파인튜닝 종류**: SFT(프롬프트-정답), instruction tuning(지시 따르기), continued pre-training(도메인 지식 주입), preference optimization(RLHF·DPO·ORPO). instruct 모델은 이미 뒤 둘을 거침
- **SFT 데이터·손실 마스킹**: 베이스 모델 채팅 템플릿에 정확히 맞춰 토큰화, 손실은 assistant 토큰에만(프롬프트는 마스킹) → "질문 외우기" 방지
- **전체 FT 비용**: 파라미터당 ~16B, 7B면 100GB+ → ZeRO-3·FSDP, gradient checkpointing, bf16, 8bit optimizer. 실무는 대부분 LoRA/QLoRA로 대체
- **파인튜닝 위험**: 파국적 망각, 정렬 비용(alignment tax), 과적합, 데이터 품질 민감, 분포 이동 취약 → 낮은 학습률·1~3 에폭·일반 데이터 혼합·안전성 재평가·조기 종료
- **평가 3축**: 타깃 과제 + 일반 능력 회귀 + 안전성. 학습 손실만 보면 과적합을 놓침
- **prompt-only baseline**: 튜닝 기법 비교의 대조군(frozen LM + 손 프롬프트, zero/few-shot). 용도는 하한선·한계 이득·기여 분리·공짜 기준점. PET/LM-BFF 스타일(템플릿+verbalizer+argmax)
- **기준선 4요소**: 평가셋(대표성·1000+·오염 방지·슬라이스·dev/test 분리) / 프롬프트 템플릿(통제 변수·말바꿔쓰기 분산) / 출력 스키마(결정적 파서·실패 처리 사전 규정·형식 오류율도 지표) / 지표(주 지표 1개 고정·micro vs macro·부트스트랩 CI·효율 축 필수)
- **프롬프트 템플릿 4구성요소**: Role(관점·전문성, 없던 능력은 안 생김) / Task(가장 중요, 목표·정의·규칙·경계 사례, primacy·recency) / Input(구분자로 경계·인젝션 방어) / Output Schema(출력 큐·structured output, 스키마 최소화, CoT는 태그 블록+최종답 줄로 절충)
- **형식 고정의 이유**: 출력 축소가 아니라 "형식은 모든 비교군 동일". 안 그러면 성능 차이에 형식 준수율 차이가 교란 변수로 섞임. 준수율 99%+ 목표, 결과표에 병기
- **가드레일 지표**: 주 지표 옆에 항상 형식 준수율·지연·비용·회귀를 같이 둠 — 주 지표만 개선처럼 보이고 이쪽에서 문제가 드러남
- **형식 준수율**: 파서가 유효 예측 뽑은 비율. SFT가 서두("Sure!") 붙여 붕괴, 긴 입력에서만 붕괴, 제약 디코딩이 정확도 깎는 사례
- **지연**: TTFT(프리필)·토큰당·전체로 분해, 평균 아닌 p50/p90/p99. few-shot+CoT는 p90 크고, LoRA+zero-shot 단답으로 대체 가능. 종료 토큰 못 내는 꼬리는 stop 시퀀스로
- **비용**: 예시당/1k건당. 32-shot의 +5%p는 입력 토큰 19배 비용 → LoRA로 흡수하면 학습 1회+추론은 zero-shot 단가. CoT의 +3%p는 출력 토큰 급증. 손익분기는 호출량이 결정
- **회귀**: 과제 내 / 일반 능력(파국적 망각) / 안전성(정렬 비용) / 형식·표현. 슬라이스별 지표·짝지은 전후 비교·회귀 스위트 고정·일반 벤치마크 전후 실행으로 포착
- **Preference alignment**: SFT 이후 선호 데이터(A가 B보다 낫다)로 모델 행동을 다듬는 후속 학습 단계. RLHF(보상 모델+PPO)·DPO(선호 데이터로 직접 최적화)·RLAIF(AI 피드백)가 대표 방법
- **RLHF 구성요소**: SFT 모델·참조 모델·보상 모델·정책·KL 페널티. PPO가 보상에서 KL 페널티를 뺀 값을 최대화하도록 정책을 반복 업데이트, 관여 모델 4개(정책·참조·보상·가치함수)
- **RLHF vs DPO**: RLHF는 보상 모델 학습+PPO 루프 2단계(불안정해지기 쉬움), DPO는 선호 데이터로 정책을 1회 손실 함수로 직접 최적화(단순·안정적, 관여 모델 2개)
- **응답정책 설계**: system(플랫폼 불변 규칙)·developer(앱 지시) 계층 구조. 범위·출력형식·톤·가드레일·모호성 처리·도구 정책을 금지보다 대안 제시, 측정 가능한 규칙, 명시적 우선순위로 작성하고 가드레일 지표로 검증
- **Baseline·Few-shot·Zero-shot 역할**: baseline은 개선폭을 재는 눈금의 0점(공정성이 핵심), few-shot은 예시로 경계를 메우는 동시에 fine-tuning이 이겨야 할 상한선 baseline, zero-shot은 순수 능력 진단+가장 싸고 빠른 실행 형태+최종 배포 지향점
- **루브릭**: 정확성·형식 준수·일관성(응답 내 모순 여부/run-to-run 변동)·불확실성(calibration) 네 축으로 개별 응답을 채점하는 다차원 기준표. LLM 심판의 system 프롬프트에 그대로 반영
- **Self-consistency**: temperature>0으로 추론 경로 N개(10~40개)를 독립 샘플링한 뒤 최종 답을 다수결. 오답은 제각각 다르게 틀리고 정답은 여러 경로가 수렴한다는 직관, 비용·지연은 N배
- **Reasoning 모델 프롬프팅**: 긴 내부 추론을 학습(RL)으로 내재화한 모델(o1·DeepSeek-R1·extended thinking)에는 CoT 지시가 불필요하거나 역효과, 추론/최종답 채널이 분리, 추론 강도는 프롬프트가 아니라 별도 설정값(reasoning effort·thinking budget)으로 조절
...
- **주요 결과**: decode 1.3~2.3× / 에이전트형일수록 안정적(단일 턴 대비 12% 이내 하락) / tail TTFT 44초 미만 vs 기준선 150초+ / 8GB 노트북에서 350억 모델 39.3 tok/s
- **차별점**: 라우팅 연산은 정확히 유지한 채 "잔여 미스를 전송할지 CPU 실행할지"를 바꿈. 실측 대역폭에서 닫힌 형태 비율로 도출해 CUDA Graph 내부 device-resident로 유지

### 하네스 엔지니어링 · [상세 →](4-LLM.md#하네스-엔지니어링-모델을-감싸는-실행-환경-설계)
...
- **Tool calling**: 모델이 구조화된 요청(JSON: 도구명+파라미터)을 텍스트 대신 출력하는 능력. 실제 실행은 하네스가 담당, 호출도 결국 assistant 메시지의 특수한 형태
