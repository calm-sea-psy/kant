...
- **대표 태스크**: 분류, 시퀀스 레이블링(NER·POS), seq2seq(번역·요약), 언어 모델링(다음 토큰 예측), 문장 임베딩·검색, 생성
...
- **어휘 크기 트레이드오프**: 작으면 시퀀스↑·임베딩 행렬↓, 크면 반대. BERT 3만 / GPT-2 5만 / Llama2 3.2만 / Llama3 12.8만 / GPT-4o 20만
...
- **프롬프트 튜닝**: 모델 freeze + 입력 앞 학습 가능한 연속 벡터(soft prompt) 수십 개만 학습. 태스크당 수 KB~MB. prefix-tuning·P-tuning v2 동류. 학습 파라미터 0.01~0.1%, 큰 모델일수록 full FT와 격차↓
- **PEFT**: 대부분 freeze + 소수만 학습하는 방법 총칭(LoRA·QLoRA·Adapter·prompt/prefix·(IA)³·BitFit). 메모리·체크포인트 절감, 망각 완화, 성능 근접. 오픈모델 커스터마이징 표준
- **전체 미세조정**: 모든 가중치 역전파 갱신. 최고 잠재 성능·근본적 행동 변경, 대신 파라미터당 ~16B 메모리, 태스크마다 전체 사본, 과적합·망각 위험
- **시도 순서**: 프롬프트(+RAG) → LoRA/QLoRA(형식·스타일·반복 태스크 한계+라벨 데이터) → 프롬프트 튜닝 계열(다태스크 초경량 스위칭) → 전체 미세조정·계속 사전학습(대규모 데이터+근본 변경)
...
- **평가 3축**: 타깃 태스크 + 일반 능력 회귀 + 안전성. 학습 손실만 보면 과적합을 놓침
...
- **프롬프트 템플릿 4구성요소**: Role(관점·전문성, 없던 능력은 안 생김) / Task(가장 중요, 목표·정의·규칙·엣지케이스, primacy·recency) / Input(구분자로 경계·인젝션 방어) / Output Schema(출력 큐·structured output, 스키마 최소화, CoT는 태그 블록+최종답 줄로 절충)
...
- **회귀**: 태스크 내 / 일반 능력(파국적 망각) / 안전성(정렬 비용) / 형식·표현. 슬라이스별 지표·짝지은 전후 비교·회귀 스위트 고정·일반 벤치마크 전후 실행으로 포착
