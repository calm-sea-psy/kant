...
자연어 처리 기초, 트랜스포머 아키텍처, 사전학습 언어모델(BERT·GPT), Hugging Face Transformers, 텍스트 분류 fine-tuning, 프롬프트·PEFT·파인튜닝 적응 방법, MoE 서빙(FreeToken), 하네스 엔지니어링.

> 출처 TIL: 260901, 260902, 260904, 260907, 260908, 260909
...
프롬프트 튜닝(prompt-tuning, soft prompt tuning)은 모델 전체를 얼리고, 입력 앞에 붙는 학습 가능한 연속 벡터(soft prompt, virtual tokens) 수십 개만 역전파로 업데이트합니다. 이 벡터는 자연어 토큰이 아니라 임베딩 공간의 벡터라서 사람이 읽을 수 없고, 태스크마다 이 작은 벡터 세트(수 킬로바이트에서 수 메가바이트)만 저장하면 됩니다. 모든 층의 key와 value에 학습 벡터를 더해 표현력을 높인 prefix-tuning, 그 후속인 P-tuning v2도 같은 갈래입니다. 학습 파라미터가 모델의 0.01에서 0.1% 수준이고, 모델이 클수록 전체 미세조정과의 성능 격차가 줄어듭니다(Lester 등 2021). 작은 모델에서는 아쉬울 수 있습니다. 위치상 PEFT의 한 종류입니다.

PEFT(Parameter-Efficient Fine-Tuning, 파라미터 효율적 미세조정)는 사전학습 가중치를 대부분 얼리고 소수의 파라미터만 학습하는 방법들의 총칭입니다. LoRA와 그 4비트 양자화 버전인 QLoRA, 층 사이에 작은 모듈을 끼우는 어댑터(adapter), 앞서 본 프롬프트·prefix 튜닝, 스케일링 벡터나 편향만 학습하는 (IA)³·BitFit이 여기 속합니다. 메모리와 연산을 크게 아끼고, 체크포인트가 작아 태스크별로 갈아끼우기 쉬우며, 파국적 망각(catastrophic forgetting)이 덜하고 성능은 전체 미세조정에 근접합니다. GPU 예산이 제한적인 도메인·스타일 적응이나 태스크 최적화에서 현재 사실상 표준이며, 자세한 내용은 다음 절에서 다룹니다.

전체 미세조정(full fine-tuning)은 태스크나 도메인 데이터로 모델의 모든 가중치를 역전파로 갱신합니다. 잠재 성능이 가장 높고 새 도메인 지식을 주입하는 것처럼 모델 행동을 근본적으로 바꿀 수 있지만, 비용이 가장 큽니다. 경사와 옵티마이저 상태까지 메모리에 올리면 Adam 기준 파라미터당 약 3배의 메모리가 들어 70억 파라미터 모델도 수십에서 수백 기가바이트의 VRAM을 요구하고, 태스크마다 전체 모델 사본이 생겨 저장과 배포가 부담스럽습니다. 작은 데이터셋에서는 과적합과 파국적 망각의 위험이 있어 보통 수천에서 수만 이상의 예시와 하이퍼파라미터 튜닝이 필요합니다.
...
비용 대비 효과는 대체로 프롬프트 엔지니어링, RAG, PEFT, 전체 미세조정 순으로 좋아서 이 순서로 시도합니다. 프롬프트 엔지니어링과 RAG로 대부분 충분하고, 형식 일관성이나 특정 스타일, 반복 태스크의 정확도에서 한계가 분명하고 라벨 데이터가 있으면 LoRA나 QLoRA로 넘어갑니다. 한 모델로 여러 태스크를 서빙하며 초경량으로 전환하는 것이 목적이면 프롬프트 튜닝 계열이 맞고, 데이터가 대규모이고 모델 행동을 근본적으로 바꿔야 하며 예산이 되면 전체 미세조정이나 계속 사전학습으로 갑니다.
...
어댑터가 LoRA에 밀린 결정적 이유는 추론 지연입니다. 프로덕션에서 LoRA는 병합으로 오버헤드가 사라지지만 어댑터는 매 순전파마다 추가 비용이 듭니다. 변형으로는 여러 어댑터를 조합하는 AdapterFusion, 순차 대신 병렬로 붙이는 parallel adapter, 활성값에 스케일링 벡터만 곱하는 (IA)³이 있습니다.
...
여기서 말하는 미세조정은 보통 전체 미세조정, 즉 베이스 모델의 가중치를 실제로 갱신해 가중치가 영구적으로 바뀐 새 체크포인트를 만드는 것을 뜻합니다. 사전학습과는 목적이 다릅니다. 사전학습은 대규모 원문을 비지도로 학습해 일반 언어 능력을 얻는 단계이고 학습률이 상대적으로 큽니다. 미세조정은 소규모의 태스크·형식·도메인 특화 데이터로 이미 학습된 표현을 살짝 미는 작업이라 학습률을 훨씬 낮게(보통 1e-5에서 5e-5) 잡고 에폭도 1에서 3회로 짧게 돌립니다.

종류는 데이터와 목적으로 나뉩니다. 지도 미세조정(supervised fine-tuning, SFT)은 프롬프트-정답 쌍으로 특정 태스크나 응답 스타일을 학습합니다. 명령어 튜닝(instruction tuning)은 다양한 태스크의 지시-응답으로 지시를 따르는 일반 능력을 키웁니다. 계속 사전학습(continued pre-training)은 도메인 원문 코퍼스를 비지도로 더 학습해 새 도메인 지식과 용어를 주입합니다. 선호 최적화(preference optimization)는 선호 비교 데이터로 사람 선호에 정렬하며 RLHF, DPO, ORPO가 여기 속합니다. 공개된 "instruct" 모델은 이미 명령어 튜닝과 선호 최적화를 거친 상태이고, 사용자는 그 위에 자기 SFT를 얹습니다.
...
파인튜닝을 하는 경우는 프롬프트와 RAG로 한계가 분명할 때(형식 일관성, 특정 스타일과 톤, 프롬프트로 표현되지 않는 암묵적 태스크), 짧은 프롬프트로 같은 성능을 내 지연과 비용을 줄이고 싶을 때, 도메인 특화 지식과 충분한 고품질 데이터가 있을 때입니다. 반대로 지식이 자주 바뀌면 RAG가 낫고, 데이터가 부족하거나 빠른 반복이 필요하거나 프롬프트 개선 여지가 남았으면 하지 않습니다. 작업 흐름은 베이스 모델 선택(base와 instruct 중), 데이터 준비(채팅 템플릿 통일, 품질 검수, 학습/검증 분리, 평가셋 오염 제거), 방법 선택(전체 미세조정과 LoRA/QLoRA 중), 하이퍼파라미터 설정, 학습하며 검증 손실 모니터링(학습 손실만 보면 과적합을 놓침), 타깃 태스크와 일반 능력 회귀와 안전성 세 축의 평가, 서빙(LoRA는 병합 후 배포, 전체 미세조정은 새 체크포인트) 순입니다.

### 4. 튜닝 기법을 비교하는 기준선과 평가 설계

여러 튜닝 기법의 성능을 비교할 때 맨 윗줄에 놓는 대조군이 prompt-only baseline입니다. 사전학습 모델을 완전히 얼리고(가중치와 경사 업데이트 0) 사람이 손으로 만든 프롬프트나 템플릿만 붙여 그대로 추론하며, 예시를 하나도 안 넣으면 zero-shot, 문맥에 데모 k개를 넣으면 few-shot입니다. 이 기준선을 쓰는 이유는 네 가지입니다. 모델이 이 태스크를 이미 얼마나 아는지 하한선을 재고, 어떤 기법이 이 기준선을 겨우 이긴다면 학습할 가치가 낮다는 신호(한계 이득)를 얻고, 성능 향상이 학습된 파라미터 덕인지 프롬프트 형식 덕인지 기여를 분리하고, 연산·데이터 비용이 0이라 항상 비교 가능한 기준점이 되기 때문입니다. 구성은 PET나 LM-BFF 스타일로, "{문장} 전체적으로 이 리뷰는 ___ 이다" 같은 템플릿에 라벨 단어를 클래스로 잇는 verbalizer(좋다는 긍정, 나쁘다는 부정)를 두고, 빈칸에 들어갈 라벨 단어들의 확률을 비교해 argmax로 채점합니다. 약점은 템플릿과 verbalizer 문구에 매우 민감해 분산이 크고, 프롬프트 엔지니어링 노력이 표준화되지 않아 기준선을 대충 만들면 다른 기법이 과대평가된다는 점입니다.

기준선을 제대로 세우려면 네 요소를 통제해야 합니다.

첫째, 평가셋입니다. 실제 관심 있는 입력 분포를 대표해야 하고, 이진 지표의 신뢰구간 폭은 대략 표본 수의 제곱근에 반비례하므로 몇 점 차이를 구분하려면 1,000개 이상이 필요합니다. 프롬프트 설계나 few-shot 예시, 나중의 학습 데이터와 겹치면 안 되고(공개 벤치마크는 사전학습 오염도 의심), 언어·난이도·카테고리·길이·엣지케이스별 슬라이스로 쪼개 어디서 실패하는지 봐야 하며, dev로 프롬프트를 반복하고 test는 거의 건드리지 않습니다. 한 번 정하면 버전을 고정합니다.
...
역할은 모델이 취할 관점과 전문성을 지정하며 보통 시스템 메시지에 들어갑니다("너는 제품 리뷰 감성 분류기다"). 효과는 있지만 한계가 분명해서 역할 부여만으로 없던 능력이 생기지는 않고, "세계 최고의" 같은 과장은 이득이 거의 없으며, 최신 instruct 모델은 롤플레이 의존도가 낮아 명확한 지시가 더 중요합니다.

지시는 실제로 무엇을 할지를 정하는 가장 중요한 부분입니다. 목표를 명령형 한 문장으로 명확히 하고, 핵심 용어와 라벨의 정의("neutral은 긍·부정 성향이 전혀 없을 때만"), 다단계면 절차, 경계와 tie-break 규칙("모호하면 X를 선택", "문장별이 아니라 전체 기준"), 길이·언어·금지 행동 같은 제약, 빈 입력이나 범위 밖 입력 같은 엣지케이스를 담습니다. 중요한 지시는 맨 앞이나 맨 뒤에 두고(primacy·recency), 부정문보다 긍정형으로 씁니다. 기준선에서 "프롬프트 엔지니어링 노력"의 대부분이 여기 들어가며, 부실하면 불공정한 기준선이 됩니다.
