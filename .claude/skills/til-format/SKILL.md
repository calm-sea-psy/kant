...
3. 마크다운 헤더 → 번호 구조 / 펜스 코드 → 4칸 들여쓰기 / 평문 표 → 마크다운 표 / `- 소제목` → 평문 라벨 또는 `ㄱ)` / LaTeX 제거 / 들여쓰기 0·4·8 정규화.
...
검사: 마크다운 헤더, 펜스 코드 블록, `$...$` LaTeX, 맨 앞 빈 줄, 구분선 없는 마크다운 표, 마커(`ㄱ)`/`a.`) 뒤 공백 누락, 표 앞뒤 빈 줄 누락. 위반을 `파일:줄  설명` 형식으로 출력하고, 하나라도 있으면 종료 코드 1.
